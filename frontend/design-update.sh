...
# ── FIX 13: Duplicate className attributes in JSX ────────────
python3 - "$SRC" << 'PY'
import sys, re, glob, os

src_dir = sys.argv[1]
total_fixed = 0
for path in glob.glob(os.path.join(src_dir, '**/*.tsx'), recursive=True):
    with open(path) as f:
        content = f.read()
    orig = content
    # Merge pairs: className="A" className="B" → className="A B"
    pattern = re.compile(r'(className="[^"]*")(\s+)(className="[^"]*")')
    while pattern.search(content):
        def merge(m):
            v1 = re.search(r'className="([^"]*)"', m.group(1)).group(1)
            v2 = re.search(r'className="([^"]*)"', m.group(3)).group(1)
            return f'className="{v1} {v2}"'
        content = pattern.sub(merge, content, count=1)
    if content != orig:
        with open(path, 'w') as f:
            f.write(content)
        total_fixed += 1
...
if total_fixed == 0:
    print("  No duplicate className found")
PY
log "Scanned & fixed duplicate className attributes"

# ── FIX 14: Literal \\n mid-line in TSX function bodies ──────
python3 - "$SRC" << 'PY'
...
for path in glob.glob(os.path.join(src_dir, '**/*.tsx'), recursive=True):
    ...
    # Detect literal \n outside of string contexts:
    # If a line (split by real newlines) contains \n not inside quotes/backtick
    lines = raw.split(b'\n')
    new_lines = []
    changed = False
    for line in lines:
        # Only look at lines that have \n NOT inside a string
        if b'\\n' in line:
            # Heuristic: if \n appears outside any quote context in function/JSX
            # Simple check: if the line itself is code-like and \n breaks a JS token
            # Safe rule: if \n appears right before whitespace+JSX (e.g. {\n  return)
            if re.search(rb'\}\s*\\n\s+return\s*\(', line):
                fixed_line = line.replace(b'\\n', b'\n')
                new_lines.extend(fixed_line.split(b'\n'))
                changed = True
                continue
        new_lines.append(line)
...
if fixed == 0:
    print("  No problematic literal \\n found")
PY
log "Scanned for literal \\n in TSX function bodies"
