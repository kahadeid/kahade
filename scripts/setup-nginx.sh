...
for f in landing app admin api; do
    [[ -f "/etc/nginx/conf.d/${f}.conf" ]] && \
        cp "/etc/nginx/conf.d/${f}.conf" "/etc/nginx/conf.d/${f}.conf.bak.${BACKUP_SUFFIX}"
done
log "Existing configs backed up (*.bak.${BACKUP_SUFFIX})"
