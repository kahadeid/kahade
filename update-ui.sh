...
# Auto-detect project root — works regardless of where script is placed or run from
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"

# Try multiple possible locations for the frontend/src folder
find_frontend_src() {
  local base="$1"
  local candidates=(
    "$base/frontend/src"
    "$base/kahade-master/frontend/src"
    "$base/../frontend/src"
    "$base/src"
  )
  for c in "${candidates[@]}"; do
    if [ -d "$c/pages" ] && [ -d "$c/components" ]; then
      echo "$c"
      return 0
    fi
  done
...
FRONTEND_SRC=$(find_frontend_src "$SCRIPT_DIR")
if [ -z "$FRONTEND_SRC" ]; then
  FRONTEND_SRC=$(find_frontend_src "$(pwd)")
fi
if [ -z "$FRONTEND_SRC" ]; then
  echo ""
  echo "  ❌ ERROR: Cannot find frontend/src directory!"
  echo ""
  echo "  Script searched in:"
  echo "    • $(pwd)/frontend/src"
  echo "    • $(pwd)/kahade-master/frontend/src"
  echo ""
  echo "  Please run this script from your project root folder."
  echo "  Example: cd ~/kahade && bash update-ui.sh"
  echo ""
  exit 1
fi

echo "  ✅ Project found at: $FRONTEND_SRC"
...
for dir in "$PAGES" "$COMPONENTS_HOME" "$COMPONENTS_LAYOUT"; do
  if [ ! -d "$dir" ]; then
