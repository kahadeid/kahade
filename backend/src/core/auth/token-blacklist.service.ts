  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (redisHost && redisPort) {
      ...
      this.redis.on("error", (err) => {
        this.logger.warn(
          `Redis error: ${(err as Error).message}. Falling back to memory storage.`,
        );
        this.redis = null;
      });
  ...
  async isBlacklisted(token: string): Promise<boolean> {
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error checking blacklist: ${(error as Error).message}`,
        );
        // Fall back to memory
      }
    }
  ...
  async blacklistToken(
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error blacklisting token: ${(error as Error).message}`,
        );
        // Fall back to memory
      }
    }
  ...
  async storeRefreshToken(
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error storing refresh token: ${(error as Error).message}`,
        );
      }
    }
  ...
  async validateRefreshToken(token: string): Promise<string | null> {
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error validating refresh token: ${(error as Error).message}`,
        );
      }
    }
  ...
  async revokeRefreshToken(
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error revoking refresh token: ${(error as Error).message}`,
        );
      }
    }
  ...
  async revokeRefreshTokenHash(
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error revoking refresh token by hash: ${(error as Error).message}`,
        );
      }
    }
  ...
  async revokeAllUserTokens(
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error revoking all user tokens: ${(error as Error).message}`,
        );
      }
    }
