  ...
  private _initializeRedis(): void {
    ...
    if (redisUrl) {
      try {
        ...
        this.logger.warn(
          `Redis initialization failed, using in-memory fallback: ${(error as Error).message}`,
        );
        this.redis = null;
      }
    } else {
  ...
  async checkAccountLock(identifier: string): Promise<LockStatus> {
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error checking lock: ${(error as Error).message}`,
        );
        // Fall back to memory
      }
    }
  ...
  async recordFailedAttempt(
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error recording attempt: ${(error as Error).message}`,
        );
        // Fall back to memory
      }
    }
  ...
  async clearFailedAttempts(identifier: string): Promise<void> {
    ...
    if (this.redis) {
      try {
        ...
        this.logger.error(
          `Redis error clearing attempts: ${(error as Error).message}`,
        );
      }
    }
