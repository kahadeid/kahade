...
export class RatingController {
  ...
  async createRating(
    ...
    const score = dto.score ?? dto.rating;
    const review = dto.review ?? dto.comment;
    ...
    const containsProfanity = review ? this.checkProfanity(review) : false;
    ...
    const rating = await this.prisma.rating.create({
      data: {
        ...
        score,
        review: review?.trim(),
        containsProfanity: review ? containsProfanity : false,
  ...
  async getUserRatings(
    ...
    return {
      ...
      data: ratings.map((r: any) => ({
        id: r.id,
        score: r.score,
        review: r.containsProfanity
          ? "[Review hidden due to policy violation]"
          : r.review,
        fromUser: r.fromUser,
        order: r.order,
      ...
      stats: {
        averageScore: stats._avg.score
          ? Number(stats._avg.score.toFixed(2))
          : 0,
  ...
  async updateRating(
    ...
    if (hoursSinceCreation > 24) {
      throw new BadRequestException(
        "Ratings can only be updated within 24 hours",
      );
    }

    const containsProfanity = dto.review
      ? this.checkProfanity(dto.review)
      : false;
  ...
  async deleteRating(
    ...
    if (hoursSinceCreation > 1) {
      throw new BadRequestException(
        "Ratings can only be deleted within 1 hour",
      );
    }
  ...
  private async updateUserReputation(userId: string): Promise<void> {
    ...
    const reputationScore = stats._avg.score
      ? Number(stats._avg.score.toFixed(2))
      : 0;
