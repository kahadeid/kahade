...
export class OrderController {
  ...
  @ApiParam({
    name: "orderNumber",
    description: "Order number (e.g., ORD-20240115-ABC123)",
  })
  @ApiResponse({ status: 200, description: "Returns order details" })
  @ApiResponse({ status: 404, description: "Order not found" })
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot update - order already accepted",
  })
  @ApiResponse({
    status: 403,
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot cancel - order already paid",
  })
  @ApiResponse({
    status: 403,
  ...
  async deleteOrder(
    @CurrentUser("id") userId: string,
    @Param("id", ParseUUIDPipe) orderId: string,
    @Query() cancelOrderDto: CancelOrderDto,
  ) {
  ...
  @Post(":id/cancel")
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: "Cancel order (preferred route)" })
  @ApiParam({ name: "id", description: "Order ID" })
  @ApiResponse({ status: 200, description: "Order cancelled successfully" })
  @ApiResponse({
    status: 400,
  ...
  @ApiResponse({
    status: 200,
    description: "Payment successful, funds locked in escrow",
  })
  @ApiResponse({
    status: 400,
    description: "Invalid order state or insufficient balance",
  })
  @ApiResponse({
    status: 403,
  ...
  @ApiResponse({
    status: 403,
    description: "Not authorized to dispute this order",
  })
  @ApiResponse({ status: 404, description: "Order not found" })
  async openDispute(
    @CurrentUser("id") userId: string,
  ...
  @Get("invite/:token")
  @ApiOperation({
    summary: "Get order details by invite token (for accepting)",
  })
  ...
  async updateComment(
    ...
    return this.orderService.updateComment(
      userId,
      orderId,
      commentId,
      updateCommentDto,
