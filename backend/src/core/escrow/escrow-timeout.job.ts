...
export class EscrowTimeoutJob {
  ...
  async handleEscrowTimeouts(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Escrow timeout job failed: ${(error as Error).message}`,
        (error as Error).stack,
      );
    }
  }
  ...
  async handleExpiredInvites(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Expired invite cleanup failed: ${(error as Error).message}`,
        (error as Error).stack,
      );
    }
  }
  ...
  async handleDataRetention(): Promise<void> {
    ...
    try {
      ...
      // Note: Actual deletion is handled by DBA based on compliance policy
      if (oldEscrows > 0) {
      ...
      this.logger.error(
        `Data retention job failed: ${(error as Error).message}`,
        (error as Error).stack,
      );
    }
  }
