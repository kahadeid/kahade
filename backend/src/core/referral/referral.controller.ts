...
@ApiTags("referral")
@Controller(["referral", "referrals"])
// Routes available under both /referral and /referrals for frontend compatibility
export class ReferralController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({ summary: "Get or generate user referral code (alias)" })
  @ApiResponse({ status: 200, description: "Returns user referral code" })
  async getReferralCodeAlias(@CurrentUser("id") userId: string) {
    return this.getReferralCode(userId);
  }

  // Shared implementation for referral code retrieval
  private async getReferralCode(userId: string) {
    ...
    return {
      ...
      shareLink: `${process.env.FRONTEND_URL || "https://kahade.id"}/register?ref=${referralCode.code}`,
    };
  }
  ...
  async getReferralStats(@CurrentUser("id") userId: string) {
    const [referralCode, usages, rewards, pendingRewards] = await Promise.all([
    ...
    const completedCount = usages.filter(
      // Eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return {
      ...
      totalEarnings: Number(rewards._sum.amountMinor || 0n) / 100,
      pendingEarnings: Number(pendingRewards._sum.amountMinor || 0n) / 100,
      // Legacy fields for backward compatibility
      code: referralCode?.code || null,
      // Eslint-disable-next-line @typescript-eslint/no-explicit-any
      activeReferrals: completedCount,
      totalRewardsEarned: Number(rewards._sum.amountMinor || 0n) / 100,
      totalRewardsClaimed: rewards._count,
      // Eslint-disable-next-line @typescript-eslint/no-explicit-any
      referrals: usages.map((u: any) => ({
        id: u.id,
        user: u.referredUser.username,
        status: u.status === "ACTIVE" ? "COMPLETED" : u.status,
        usedAt: u.usedAt,
  ...
  async getReferralList(
    ...
    const [usages, total] = await Promise.all([
      ...
      // Eslint-disable-next-line @typescript-eslint/no-explicit-any
      this.prisma.referralUsage.count({ where: { referrerId: userId } }),
      // Eslint-disable-next-line @typescript-eslint/no-explicit-any
    ]);
    // Eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    // Eslint-disable-next-line @typescript-eslint/no-explicit-any
    const referrals = usages.map((u: any) => {
      // Eslint-disable-next-line @typescript-eslint/no-explicit-any
      const claimedReward = u.rewards.find((r: any) => r.status === "CLAIMED");
      // Eslint-disable-next-line @typescript-eslint/no-explicit-any
      const pendingReward = u.rewards.find((r: any) => r.status === "PENDING");
      const rewardAmount =
        claimedReward?.amountMinor || pendingReward?.amountMinor || 0n;

      return {
        id: u.id,
        referredUser: {
          username: u.referredUser.username,
          createdAt: u.referredUser.createdAt,
        },
        status: u.status === "ACTIVE" ? "COMPLETED" : u.status,
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      try {
        ...
        if (
          ...
          this.logger.warn(`Referral code collision for ${code}. Retrying...`);
          continue;
        }
