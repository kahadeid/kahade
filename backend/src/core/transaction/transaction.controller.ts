...
export class TransactionController {
  ...
  @Get("invite/:token")
  @ApiOperation({ summary: "Get transaction by invite token" })
  @ApiParam({ name: "token", description: "Invite token" })
  @ApiResponse({ status: 200, description: "Returns transaction invite data" })
  @ApiResponse({ status: 404, description: "Invite token invalid or expired" })
  async findByInviteToken(@Param("token") inviteToken: string) {
    return this.transactionService.findByInviteToken(inviteToken);
  }
  ...
  @ApiOperation({ summary: "Accept transaction invitation via invite token" })
  @ApiBody({ type: AcceptInviteDto })
  @ApiResponse({ status: 200, description: "Transaction accepted" })
  @ApiResponse({ status: 400, description: "Cannot accept invite" })
  async acceptInvite(
    @CurrentUser("id") userId: string,
    @Body() dto: AcceptInviteDto,
  ) {
    return this.transactionService.acceptByInvite(dto.inviteToken, userId);
  }
  ...
  @ApiParam({ name: "id", description: "Transaction ID" })
  @ApiBody({
    schema: { properties: { reason: { type: "string", maxLength: 500 } } },
  })
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot pay - insufficient balance or invalid status",
  })
  ...
  @Get(":id/ratings")
  @ApiOperation({ summary: "Get transaction ratings" })
  @ApiParam({ name: "id", description: "Transaction ID" })
  @ApiResponse({ status: 200, description: "Returns transaction ratings" })
  async getRatings(@Param("id") id: string, @CurrentUser("id") userId: string) {
    return this.transactionService.getRatings(id, userId);
  }
  ...
  @ApiOperation({ summary: "Submit delivery proof (seller)" })
  ...
  @ApiBody({
    schema: {
      properties: {
        ...
        courier: { type: "string", maxLength: 100 },
  ...
  async submitDeliveryProof(
    ...
    const normalizedFileUrls = Array.isArray(data.fileUrls)
      ? data.fileUrls
      : data.fileUrls
        ? [data.fileUrls]
        : undefined;
    if (data.courier && data.courier.length > 100) {
      throw new BadRequestException(
        "Courier name must not exceed 100 characters",
      );
    }
    if (data.trackingNumber && data.trackingNumber.length > 100) {
      throw new BadRequestException(
        "Tracking number must not exceed 100 characters",
      );
    }
    if (data.notes && data.notes.length > 1000) {
      throw new BadRequestException("Notes must not exceed 1000 characters");
    }
    return this.transactionService.submitDeliveryProof(
      id,
      userId,
      { ...data, fileUrls: normalizedFileUrls },
      file,
    );
  }
  ...
  @ApiParam({ name: "id", description: "Transaction ID" })
  @ApiResponse({ status: 200, description: "Returns delivery proof" })
  async getDeliveryProof(
    @Param("id") id: string,
    @CurrentUser("id") userId: string,
  ) {
    return this.transactionService.getDeliveryProof(id, userId);
  }
}
