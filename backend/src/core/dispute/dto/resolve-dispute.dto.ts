...
export class ResolveDisputeDto {
  ...
  @ApiProperty({
    ...
    required: false,
    description:
      "Amount in minor units (cents) to give to seller. Required for SPLIT_SETTLEMENT.",
  })
  ...
  @ApiProperty({
    ...
    required: false,
    description:
      "Amount in minor units (cents) to refund to buyer. Required for SPLIT_SETTLEMENT.",
  })
  @ValidateIf((o) => o.decision === "SPLIT_SETTLEMENT")
  @IsNotEmpty({
    message: "Buyer refund amount is required for split settlement",
  })
  @IsString()
  @Matches(/^[0-9]+$/, {
    message: "Buyer refund amount must be a numeric string",
  })
  buyerRefundMinor?: string;

  @ApiProperty({
    example:
      "After reviewing all evidence submitted by both parties, the buyer's claim is valid. The product description was misleading.",
