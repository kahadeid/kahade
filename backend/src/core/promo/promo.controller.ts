...
export class PromoController {
  ...
  async getVoucherHistory(
    ...
    return this.promoService.getUserVoucherHistory(
      userId,
      page || 1,
      limit || 20,
    );
  ...
  @ApiBearerAuth("JWT-auth")
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: "Validate promo code (alias for vouchers)" })
  async validatePromoAlias(
    @CurrentUser("id") userId: string,
    @Body()
    dto: {
      code: string;
    ...
    const voucher = result.voucher;
    let discountMinor: bigint;
    const orderAmountMinor = BigInt(amountMinor);
    ...
    return {
      ...
      preview: {
        originalMinor: amountMinor.toString(),
        discountMinor: discountMinor.toString(),
        finalMinor: (orderAmountMinor - discountMinor).toString(),
      },
    };
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({ summary: "Apply promo code (alias for vouchers)" })
  async applyPromoAlias(
    @CurrentUser("id") userId: string,
    @Body()
    dto: {
      code: string;
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({ summary: "Get available promos (alias for vouchers)" })
  async getAvailablePromos(@CurrentUser("id") userId: string) {
    return this.promoService.getAvailableVouchersForUser(userId);
  }
  ...
  private resolveAmountMinor(
    ...
    const resolved = amountMinor ?? orderAmount;
    if (resolved === undefined || resolved === null) {
      throw new BadRequestException(
        "orderAmount or amountMinor must be provided",
      );
    }
    if (Number.isNaN(Number(resolved)) || Number(resolved) <= 0) {
      throw new BadRequestException("Amount must be a positive number");
    }
    return Math.round(Number(resolved));
  }
}
