...
describe("BankRepository", () => {
  ...
  describe("create", () => {
    it("should create bank account with encrypted data", async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const _result = await repository.create(createData);
      ...
      const createCall = (prismaService.bankAccount.create as jest.Mock).mock
        .calls[0][0];
      expect(createCall.data.accountNumberEnc).not.toBe(
        createData.accountNumber,
      );
      expect(createCall.data.accountNameEnc).not.toBe(
        createData.accountHolderName,
      );
    });
  ...
  describe("softDelete", () => {
    it("should soft delete bank account", async () => {
      (prismaService.bankAccount.updateMany as jest.Mock).mockResolvedValue({
        count: 1,
      });
      ...
      const updateCall = (prismaService.bankAccount.updateMany as jest.Mock)
        .mock.calls[0][0];
  ...
  describe("getDecryptedAccount", () => {
    it("should return account with decrypted data", async () => {
      ...
      (prismaService.bankAccount.create as jest.Mock).mockImplementation(
        (data) => {
          encryptedAccount = {
            ...mockBankAccount,
