...
# FIX-PERM: www-data (nginx) harus bisa baca frontend/dist
# chmod 750 di atas memblokir nginx → 'Permission denied' → HTTP 500
usermod -aG "$DEPLOY_USER" www-data
chmod 755 "$DEPLOY_DIR"
chmod 755 "$DEPLOY_DIR/frontend"
log_success "nginx (www-data) ditambahkan ke group $DEPLOY_USER → bisa baca frontend/dist"
...
# FIX-27: BANK_ENCRYPTION_KEY & KYC_ENCRYPTION_KEY required by BankRepository & KycController
# Both validate 32-byte hex string at startup → crash if missing
BANK_ENCRYPTION_KEY="$(openssl rand -hex 32)"   # 64 hex chars = 32 bytes
KYC_ENCRYPTION_KEY="$(openssl rand -hex 32)"    # 64 hex chars = 32 bytes
...
# FIX-27: Required by BankRepository & KycController — crash at startup if missing
...
# Cara setup: https://www.zoho.com/mail/help/smtp-configuration.html
# 1. Login ke mail.zoho.com → Settings → Mail Accounts → verifikasi@kahade.id
# 2. Settings → Security → App Passwords → buat App Password khusus untuk ini
# 3. Isi SMTP_PASS dan SMTP_PASSWORD dengan App Password tersebut (BUKAN password login Zoho)
...
# ── Backend: Migrations ───────────────────────────────────────────────────────
# FIX-28: 'set -a; source .env' gagal jika ada value dengan spasi (BACKUP_SCHEDULE="0 2 * * *")
# → Prisma tidak dapat DATABASE_URL → migrate abort diam-diam
# Solusi: extract DATABASE_URL langsung dari file, pass via env variable eksplisit
...
" || die "DATABASE MIGRATION GAGAL — deploy dibatalkan. Cek koneksi DB dan schema."
log_success "Semua migrasi database berhasil diterapkan"

# FIX-DISPUTE-DESC: Kolom disputes.description ada di schema.prisma tapi tidak di migration init
# Prisma generate tidak otomatis menambahkan kolom yang terlewat → runtime error saat query disputes
# Solusi: ALTER TABLE idempotent (IF NOT EXISTS) agar aman di-run ulang
...
" 2>/dev/null \
  ...
  || log_warning "disputes.description hotfix gagal — jalankan manual: ALTER TABLE disputes ADD COLUMN IF NOT EXISTS description TEXT NOT NULL DEFAULT '';"
...
async function main() {
  ...
  // FIX: Admin needs a wallet to use the platform
...
# FIX-NGINX-LIMIT: worker_connections 2048 > default open file limit 1024 → warn di setiap reload
# Solusi: override LimitNOFILE via systemd drop-in
...
systemctl restart nginx
log_success "Nginx LimitNOFILE dinaikkan ke 65536 (no more worker_connections warn)"
...
# FIX: Source env file into shell BEFORE starting PM2 so all env vars are
# inherited by PM2 worker processes. env_file in ecosystem config requires PM2 >= 5,
# but sourcing here works with any PM2 version.
sudo -u "$DEPLOY_USER" bash -c "
  set -a
  source '$ENV_FILE'
  set +a
...
# ── SSL: Cloudflare (certbot tidak diperlukan) ────────────────────────────────
# SSL ditangani oleh Cloudflare proxy — tidak perlu certbot di server.
# Pastikan di Cloudflare dashboard:
#   DNS → Records: A kahade.id, A api.kahade.id, A www.kahade.id → semua Proxied (🟠)
#   SSL/TLS → Overview → Mode: "Full" (bukan Flexible)
log_success "SSL: dihandle Cloudflare — certbot tidak diperlukan"
...
echo "==========================================================================="
echo
echo "  🌐 Frontend:  http://${DOMAIN}   (HTTPS via Cloudflare)"
...
echo "     3. Cloudflare: SSL/TLS → Full | DNS semua record → Proxied (🟠)"
